...
#define DEVNET_QLINKLOOPTIME  120  //2S/200MS = 10
...
int d_wifi_matchSsid_0isSame(char *Cssid){
	...
	DPRI_INFO("t_wireless_param.a_SSID is %s\n",t_net_param.t_wireless_param.a_SSID);
	if( strlen(t_net_param.t_wireless_param.a_SSID) == 0){		
		DPRI_INFO("t_wireless_param.a_SSID is null.len is 0\n");
		return -1;
	}
...
int d_dualNetCheck(u32 exeCount){
	...
	if(pwifimsg->netstatebit != pwifimsg->netstatebit_last){
		...
		if( (pwifimsg->netstatebit &0x04) == 0x04)
			...
			DPRI_INFO("will be normal wifi,[%d]\n",pwifimsg->netstatebit);
